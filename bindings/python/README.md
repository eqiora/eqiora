...
Eqiora `0.1.0` supports ordinary-GIL CPython 3.11–3.14 on
manylinux x86-64:

```console
uv venv --python 3.13 .venv
uv pip install --python .venv/bin/python eqiora==0.1.0
```
...
uv pip install --python .venv/bin/python "eqiora[gmsh]==0.1.0"
```
...
`uv pip install --python .venv/bin/python "eqiora[gmsh,matplotlib]==0.1.0"`.
...
instructions when trying features newer than `0.1.0`.
...
`0.1.0` is a pre-1.0 release. Python APIs and saved-file formats may change;
