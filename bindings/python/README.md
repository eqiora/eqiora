...
**Release — `0.1.1`.** See [Capabilities](https://eqiora.org/capabilities/)
for available models, methods, and platforms.

## Install

Eqiora `0.1.1` supports ordinary-GIL CPython 3.11–3.14 on
manylinux x86-64:

```console
uv venv --python 3.13 .venv
uv pip install --python .venv/bin/python eqiora==0.1.1
```
...
uv pip install --python .venv/bin/python "eqiora[gmsh]==0.1.1"
```
...
```console
uv pip install --python .venv/bin/python "eqiora[torch]==0.1.1"
uv pip install --python .venv/bin/python "eqiora[jax]==0.1.1"
uv pip install --python .venv/bin/python "eqiora[matplotlib]==0.1.1"
```

The exact-cylinder pressure example combines the mesher and plot adapter:
`uv pip install --python .venv/bin/python "eqiora[gmsh,matplotlib]==0.1.1"`.
...
instructions when trying features newer than `0.1.1`.
...
`0.1.1` is a pre-1.0 release. Python APIs and saved-file formats may change;
